...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Train and validate a model on CIFAR-10')
    parser.add_argument('--batch_size', type=int, default=256, help='batch size')
    parser.add_argument('--epochs', type=int, default=50, help='number of epochs')
    parser.add_argument('--lr', type=float, default=0.01, help='learning rate')
    parser.add_argument('--model', type=str, default='resnet50', help='model to use')
    parser.add_argument('--wandb_pr_name', type=str, default='wandb_pr', help='wandb project name')
    parser.add_argument('--n_gpus', type=int, default=1, help='Number of allocated GPUs')
...
def main():
    ...
    validate_dataset = datasets.CIFAR10(root='./data', train=False, download=True, transform=preprocess)

    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    validate_loader = DataLoader(validate_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        # save a checkpoint
        checkpoint_filename = f"{cache_path}/checkpoints/checkpoint_epoch_{epoch+1}.pth.tar"
        save_checkpoint({
            'epoch': epoch + 1,
